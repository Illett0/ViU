...
### タイムラインデータをお持ちでない場合（写真のみモード）

Googleタイムラインのエクスポートを用意するのが難しい場合でも、位置情報付きの写真フォルダだけを連携すれば、撮影地点を地図にプロットして閲覧できます。起動画面の「タイムラインなしで写真だけ見る」から利用できます。都道府県・市区町村の制覇状況を示す色分けは行われません（訪問データ自体が存在しないため）が、都道府県・市区町村の境界線や写真のクラスタ表示・ギャラリー・ライトボックス・除外ゾーンなど、写真連携機能はそのまま使えます。経路マップ・移動統計・年表・タイムラプス再生のタブは、意味のあるデータが存在しないため非表示になります。あとからヘッダーの「ファイルを開く」で実際のタイムラインファイルを読み込めば、通常モードにそのまま移行できます。
...
- **写真のみモード**（issue #21、タイムライン不保持の人向け）: タイムラインファイルを開かずに、起動画面の「タイムラインなしで写真だけ見る」から写真フォルダの連携だけで利用開始できます。都道府県・市区町村の境界表示、写真のクラスタ表示・ギャラリー・ライトボックス、除外ゾーンはそのまま使えますが、訪問データが存在しないため制覇マップの塗り分けは行われず（全国グレー表示）、経路マップ・移動統計・年表・タイムラプス再生のタブは非表示になります。ヘッダーの「ファイルを開く」から実際のタイムラインファイルを読み込めば通常モードに移行できます。
...
- 写真のみモード（issue #21）: 起動画面の2つのボタンが両方表示されること、「タイムラインなしで写真だけ見る」から写真フォルダ連携・除外ゾーン設定（提案は出ない）を経て地図画面に到達すること、経路マップ・年表・移動統計タブとクラスタ距離スライダー・タイムラプス操作が非表示になること、全国地図が全グレーで表示されつつクリック可能でパンくず・市区町村制覇率の分母が正しいこと（都道府県・市区町村の参照リストが空でなく実データであることの回帰確認）、写真レイヤーが初期表示ONで写真ポップアップの地名が正しく解決されること、年月フィルタに写真の撮影年が反映されること、ヘッダーの「ファイルを開く」から実際のタイムラインを読み込むと非表示だったタブが復活し通常モードに戻ることを確認
